console.log("ANONYBOT Made by: Anony");
...
var reconnected = function(){
	console.log("AnonyBOT Got disconnected. Reconnecting...");
    var socket = io("https://kwr-production.up.railway.app/")
    socket.emit('login',{name:'AnonyBOT (+help)'})
...
var commands = {
    help:function(){
        return "<h2>+AnonyBOT, a fork of b!help.</h2><h3>Commands:</h3>+help, +echo [text], +join [user], +burn, +drunk [text], +clickbait [text]"
    },
    echo(txt){
        if(txt.startsWith('+')){
    ...
    join(txt){
        if(cool){
            ...
            var sock = io("https://kwr-production.up.railway.app/")
